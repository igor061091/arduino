...
const int digitalInPin = 3; //  In sensor
const int digitalPinGnd = 4; // GND sensor
const int digitalPinVcc = 5; // VCC sensor
...
void Handler_ISR() 
{
  ...
  //Timer_ResetImpulse=5;
}

bool timerHandler_RelayDalay(void *) {
  Serial.println("timerHandler_RelayDalay");
...
{
  Serial.begin(115200);
  Serial.print("IP address start look ");
  ...
  //attachInterrupt(digitalPinToInterrupt(digitalInPin), Handler_ISR, FALLING);
  //timer.every(1000, timerHandler_ImpulseCheck);
  // настройки закончены
